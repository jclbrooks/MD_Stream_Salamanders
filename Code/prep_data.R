...
########################################################
# MSOM - beta1: pH , alpha1: time_min - add covariance with occupancy and detection
sink("Code/JAGS/multi_spp_occ4.txt")
cat("
    model {
...
out4 <- jags(data = data_list, inits = inits, params, model.file = "Code/JAGS/multi_spp_occ4.txt", 
            ...
            parallel = TRUE,
            bugs.format = TRUE)
# beta1: ec, alpha1: water
out2 <- jags(data = data_list, inits = inits, params, model.file = "Code/JAGS/multi_spp_occ2.txt", 
            n.chains = nc,
            n.burnin = nb,
            n.iter = ni,
            parallel = TRUE,
            bugs.format = TRUE)
# beta1: ec, alpha1: water w/ added covariance of occupancy with detection
out3 <- jags(data = data_list, inits = inits, params, model.file = "Code/JAGS/multi_spp_occ3.txt", 
            n.chains = nc,
            n.burnin = nb,
            n.iter = ni,
            parallel = TRUE,
...
################
# out1 vs out4 - rho (out4) seems less informative but doesn't mess things up too much
# out2 vs out3 - rho (out3) is non-informative and doesn't allow models to converge
